...
function fetch_env_vars() {
    ...
    GRAPHQL_API_URL=$(aws ssm get-parameter --name /${AWS_BRANCH}/service/amplify/api/url --query 'Parameter.Value' --output text &)
...
function print_vars() {
    ENV_VARS=(AWS_DEFAULT_REGION AWS_BRANCH FLIGHT_TABLE_NAME BOOKING_TABLE_NAME STACK_NAME DEPLOYMENT_BUCKET_NAME GRAPHQL_API_ID SHARED_LIBS_LAYER VUE_APP_StripePublicKey VUE_APP_PaymentChargeUrl LOYALTY_TABLE_NAME LOYALTY_QUEUE_URL GRAPHQL_API_URL)
